...
router.get('/list', async function(req, res, next) {
	...
	try {
		...
		Products.find({}, {}, { skip: (page - 1) * itemsPerPage, limit: itemsPerPage }, (err, list) => {
			if (err) {
				return res.redirect('/');
			} else {
...
router.post('/modify', upload.single('img'), check('price').isCurrency(),
	//validation
	checkSchema({
		name: {
			notEmpty: {
...
router.post('/add', upload.single('img'), check('price').isCurrency(),
	//validation
	checkSchema({
		name: {
			notEmpty: {
